...
sed -e '/^$/d;/\\x00/d'`,
  ...
  startAP: 'ifconfig wlan0 10.100.0.1/24 up && killall dnsmasq &> /dev/null && sleep 3 && dnsmasq -C /opt/wifi-setup/scripts/dnsmasq.conf && hostapd /opt/wifi-setup/scripts/hostapd.conf && systemctl start udhcpd',
  //startAP: '/opt/wifi-setup/startap.sh',
  // Stop broadcasting an AP and attempt to reconnect to local wifi
  stopAP: 'systemctl stop dnsmasq && systemctl stop udhcpd && systemctl stop hostapd && ifconfig wlan0 0.0.0.0',
